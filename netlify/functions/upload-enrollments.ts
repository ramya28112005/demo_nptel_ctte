...
export const handler: Handler = async (event) => {
  try {
    if (event.httpMethod === 'POST') {
      const { semester_id, enrollments, base64, fileName } = JSON.parse(event.body || '{}');
      ...
      return {
        ...
        body: JSON.stringify({ success: true, rows: enrollments.length })
      };
    } else {
      return {
        statusCode: 405,
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: 'Failed to upload enrollments' })
    };
  }
};